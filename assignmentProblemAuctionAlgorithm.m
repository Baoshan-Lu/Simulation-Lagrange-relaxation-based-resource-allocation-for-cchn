...
while (epsilon >= 1)
    ...
    while (any(isnan(assignments)))
        ...
        %% bidding phase 投标阶段
        % find unassigned indices 找出未分配目标
        unassignedIdx = find(isnan(assignments));
%         unassignedIdx
        nUnassigned = numel(unassignedIdx);%计算个数
%         nUnassigned
%         prices
        % find best and second best objects 找出最佳卖品和次佳卖品
        AijMinusPj = bsxfun(@minus, A(unassignedIdx,:), prices');%价格
%         AijMinusPj
...
%         AijMinusPj
        wi = max(AijMinusPj, [], 2);%将同一行的不同列的最大值提取出来
%         wi
        % compute bids
        bids = nan(nUnassigned,1);
        for i=1:nUnassigned
            bids(i) = A(unassignedIdx(i),viIdx(i)) - wi(i) + epsilon;%减去最高价，所有人开始增加价格
        end
%         bids
        
        %% assignment phase 指派阶段
        objectsThatHaveBeenBiddedFor = uniqueFcn(viIdx);
%         objectsThatHaveBeenBiddedFor
        for uniqueObjIdx=1:numel(objectsThatHaveBeenBiddedFor)
            currObject = objectsThatHaveBeenBiddedFor(uniqueObjIdx);
%             currObject
            personssWhoGaveBidsForJ = find(viIdx==currObject);%找到对应出价的人
%             personssWhoGaveBidsForJ
            [prices(currObject), idx] = max(bids(personssWhoGaveBidsForJ));%找出出价最高的买家
%             prices
%             idx
            personWithHighestBid = unassignedIdx(personssWhoGaveBidsForJ(idx));%价高者得到目标
%             personWithHighestBid
            % remove previous assignment and store new assignment (the person
            % with highest bid)
            assignments(assignments==currObject) = nan;%竞争失败，指派为空
            assignments(personWithHighestBid) = currObject;%价高者得到目标
